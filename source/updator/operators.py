...
# 업데이트 체크
class MDAD_OT_CheckUpdate(Operator):
    bl_idname = "mdad.check_update"
    bl_label = "업데이트 체크"
    ...
    def execute(self, context):
        try:
            url = (
                "https://api.github.com/repos/DPN-dpn/modder_adapter/releases/latest"
            )
            with urllib.request.urlopen(url, timeout=5) as response:
                data = json.loads(response.read().decode())
                latest_version = data.get("tag_name", "").lstrip("v")
                context.scene["mdad.latest_version"] = latest_version
                # 현재 버전 가져오기
                from ... import bl_info

                current_version = ".".join(map(str, bl_info["version"]))
                context.scene["mdad.current_version"] = current_version
                context.scene["mdad.show_restart"] = False
                if latest_version and latest_version != current_version:
                    context.scene["mdad.update_available"] = True
                    self.report(
                        {"INFO"}, f"업데이트 가능: {current_version} → {latest_version}"
                    )
                else:
                    context.scene["mdad.update_available"] = False
...
# 업데이트 실행
class MDAD_OT_DoUpdate(Operator):
    bl_idname = "mdad.do_update"
    bl_label = "업데이트"
    ...
    def execute(self, context):
        temp_dir = None
        try:
            url = "https://api.github.com/repos/DPN-dpn/modder_adapter/releases/latest"
            with urllib.request.urlopen(url, timeout=5) as response:
                data = json.loads(response.read().decode())
            ...
            context.scene["mdad.show_restart"] = True
            _redraw_ui_regions(context)
            return {"FINISHED"}
...
# GitHub 이동
class MDAD_OT_OpenGithub(Operator):
    bl_idname = "mdad.open_github"
    bl_label = "GitHub"
    ...
    def execute(self, context):
        ...
        webbrowser.open("https://github.com/DPN-dpn/modder_adapter")
        self.report({"INFO"}, "modder_adapter GitHub 페이지를 엽니다.")
        return {"FINISHED"}

# Wiki 이동
class MDAD_OT_OpenWiki(Operator):
    bl_idname = "mdad.open_wiki"
    bl_label = "Wiki"
    ...
    def execute(self, context):
        ...
        webbrowser.open("https://github.com/DPN-dpn/modder_adapter/wiki")
        self.report({"INFO"}, "modder_adapter 위키 페이지를 엽니다.")
        return {"FINISHED"}
